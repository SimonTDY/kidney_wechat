...
.controller('DoctorDiagnoseCtrl', ['Task', '$scope', 'Storage', 'ionicDatePicker', 'Patient', '$state', '$ionicLoading', function (Task, $scope, Storage, ionicDatePicker, Patient, $state, $ionicLoading) {
  ...
  $scope.showDateTitle = function (type) {
    // console.log(type)
    var title = ''
    if (type == 1) { title = '手术日期' } else if (type == 5) { title = '插管日期' } else if (type == 6) { title = '开始日期' }
    return title
  }
...
// 健康详情--PXY
.controller('HealthDetailCtrl', ['ionicDatePicker', '$scope', '$state', '$ionicHistory', '$ionicPopup', '$stateParams', '$ionicPopover', '$ionicModal', '$ionicScrollDelegate', '$ionicLoading', '$timeout', 'Dict', 'Health', 'Storage', 'Camera', 'wechat', '$location', 'CONFIG', function (ionicDatePicker, $scope, $state, $ionicHistory, $ionicPopup, $stateParams, $ionicPopover, $ionicModal, $ionicScrollDelegate, $ionicLoading, $timeout, Dict, Health, Storage, Camera, wechat, $location, CONFIG) {
  $scope.barwidth = 'width:0%'
  var patientId = Storage.get('getpatientId')
  // var patientId = 'U201702071766'   //测试ID
...
.controller('VaryMessageCtrl', ['$scope', '$state', '$ionicHistory', 'Storage', function ($scope, $state, $ionicHistory, Storage) {
  ...
  if (messageType == 'ZF') { $scope.avatar = 'payment.png' } else if (messageType == 'JB') { $scope.avatar = 'alert.png' } else if (messageType == 'RW') { $scope.avatar = 'task.png' } else if (messageType == 'BX') { $scope.avatar = 'security.png' }
